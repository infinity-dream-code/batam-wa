...
EOD;
    ...
    private function kirimPesan(Request $request, $nomor, $pesan, $no, $nama)
    {
        ...
        $dbTraffic = new \mysqli("10.99.23.26", "root", "Smartpay1ct", "farrelep_broadcaster");
    
        $apiKeyResult = $dbTraffic->query("SELECT GetWAApiSecret('Yogya_Muallimaat')");
        if (!$apiKeyResult) {
            ...
            try {
                ...
                if ($response->successful()) {
                    ...
                    try {
                        ...
                        $functionQuery = "SELECT SentWA('Yogya_Muallimaat', '" . $nomor . "', '" . $clientNumberKey . "', '" . $escapeMessage . "')";
        ...
        try {
            ...
            LogWhatsappsModel::create([
                ...
                'nama' => $nama,
                'response' => is_string($response) ? $response : ($response?->body() ?? 'no response'),
            ]);
