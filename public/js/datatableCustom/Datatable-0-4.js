...
function enableFormSubmit(idTable, formId) {
...
function addCustomNumberFormat(xlsx, numberFormat) {
    ...
    // const rupiahFormat = 'Rp.\\ #,##0.00;[Red]Rp.\\ -#,##0';
    // const rupiahFormat = '[$$-en-AU]#,##0.00;[Red]-[$$-en-AU]#,##0.00';
    // const rupiahFormat = '[$Rp. ]#,##0;[Red]-[$Rp. ]#,##0';
    const rupiahFormat = '"Rp."* #,##0;[Red]-"Rp."* #,##0;"Rp."* -';
    // const rupiahFormat = '!R!p!. #,##0.00;[Red]!R!p!. - #,##0.00';
    ...
    DT[`${options.tableId}`] = idTable.DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                DT[`${options.tableId}`].totalData = json.totals;
                return json.data;
                ...
                if (error === "timeout") {
                    controller.abort();
                    errorAlert("Proses terlalu lama, silahkan coba lagi");
                    $('.dataTables_processing').hide();
                    enableFormSubmit(idTable, options.formId)
                } else {
        ...
        drawCallback: function (settings) {
            ...
            if (options.total) {
                // const searchableColumns = options.dataColumns.filter(item => item.totalColumn === true);
                const searchableColumns = Object.fromEntries(
                    Object.entries(options.dataColumns).filter(([key, item]) => item.totalColumn === true)
                );
                if (searchableColumns) {
                    const columnLength = options.dataColumns.length;
                    const currentFooter = $(`#${options.tableId} tfoot`);
                    const moneyTypes = ['currency', 'money'];

                    if (currentFooter.length === 0 && DT[`${options.tableId}`].totalData) {
                        idTable.append('<tfoot></tfoot>');
                        5
                    }
                    currentFooter.find('.total-row').remove();

                    function additiveInverse(x) {
                        return -x;
                    }

                    Object.entries(searchableColumns).forEach(([key, item]) => {
                        let data = DT[`${options.tableId}`].totalData[item.data] ?? 0;
                        if (item.columnType && moneyTypes.includes(item.columnType)) {
                            data = new Intl.NumberFormat('id-ID', {
                                style: 'currency',
                                currency: 'IDR',
                            }).format(data)
                        }
                        currentFooter.append(`
                            <tr class="total-row" style="">
                                <th class="fixed-column-0" colspan="${columnLength - parseInt(key)}">Total ${item.name}</th>
                                <th class="fixed-column-1" colspan="${parseInt(key) - 2}">${data}</th>
                                </tr>
                        `)
                    });
                }
            ...
            success: function (data) {
                $.each(data, function (index, column) {
                            ...
                            case 'boolean':
                                renderFunc = function (data, type, row) {
                                    if (type === 'display' || type === 'filter') {
                                        ...
                                        let trueVal = column.trueVal ?? 'Sukses';
                                        let falseVal = column.falseVal ?? 'Gagal';
                                        const currentColumnData = column.data?.toLowerCase() || "";
                                        if (currentColumnData.includes("cicil")) {
                                            trueVal = "Ya";
                                            falseVal = "Tidak";
                                        }
                                        if (data === "1" || data === 1 || data === true) {
                    ...
                    options.dataColumns.push({
                        ...
                        totalColumn: column.totalColumn ?? false,
                    })
                })
